...
import IOniPair from '../buildOni/OniPair.json'
...
describe('OniRouter02', () => {
  ...
  it('getAmountOut', async () => {
    ...
    await expect(router.getAmountOut(bigNumberify(0), bigNumberify(100), bigNumberify(100))).to.be.revertedWith(
      'OniLibrary: INSUFFICIENT_INPUT_AMOUNT'
    )
  ...
  it('getAmountIn', async () => {
    ...
    await expect(router.getAmountIn(bigNumberify(0), bigNumberify(100), bigNumberify(100))).to.be.revertedWith(
      'OniLibrary: INSUFFICIENT_OUTPUT_AMOUNT'
    )
...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    await fixture.factoryV2.createPair(DTT.address, WBNB.address)
    const pairAddress = await fixture.factoryV2.getPair(DTT.address, WBNB.address)
    pair = new Contract(pairAddress, JSON.stringify(IOniPair.abi), provider).connect(wallet)
  })
